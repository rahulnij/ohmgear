...
class VacationCardViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, vacationcard_id=None, call_from_function=None):
        ...
        serializer = VacationTripSerializer(
            vacationtrip_duplicate_data, many=True)
        data = {}
    ...
    def create(self, request, call_from_func=None, offline_data=None):
        ...
        VacationTripserializer = VacationTripSerializer(
            data=data, context={'request': request})
    ...
    def post(self, request):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                # id
                VacationTrip.objects.filter(user_id=request.user, vacationcard_id__in=sourceVacationCardIds).update(
                    vacationcard_id=destVacationCardId)
                BusinessCardVacation.objects.filter(user_id=request.user, vacationcard_id__in=sourceVacationCardIds).update(
                    vacationcard_id=destVacationCardId)

                # remove source vacation card once it trips done
                VacationCard.objects.filter(
                    user_id=request.user, id__in=sourceVacationCardIds).delete()
