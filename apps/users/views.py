...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None):
        ...
        serializer = UserSerializer(
            messages, data=request.DATA, partial=True, context={'request': request})
        if serializer.is_valid():
            serializer.save()
#            #---------------- Set the password -----------#
...
#
                ...
                sociallogin = SocialLogin(
                    user_id=data['id'], social_media_login_id=social_id, social_type_id=social_type_id)
...
#       reset_password_key = request.DATA["reset_password_key"]
        ...
        #--------------------  Get the reset password key and redirect to mobile : Processed when forgot password mail link clicked----------------------#
            ...
            if request.device:
                from django.http import HttpResponse
                response = HttpResponse(
                    "kinbow://?resetPasswordKey=" + reset_password_key, status=302)
    ...
    if request.method == 'POST':
        ...
        # ----------- restet password and send the email------------------#
            ...
            if profile:
                user = model_to_dict(profile.user)
                BaseSendMail.delay(
                    user, type='account_confirmation', key=profile.activation_key)
...
#        data.append(default_email)
    ...
    def send_verification_code(self, request):
        ...
        try:
            ...
            if user_id:
                UserEmail.objects.filter(user_id=request.user.id, email=request.DATA.get(
                    'email')).update(verification_code=activation_key)
                BaseSendMail.delay(
