...
class GroupContactsViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        for contacts in group_contacts:
            ...
            group_contact_data_exist = self.queryset.filter(user_id=request.user, folder_contact_id=data[
                                                            'folder_contact_id'], group_id=data['group_id'])
        ...
        serializer = self.serializer_class(
            data=tempContainer, many=True, context={'contact_data': 1})
        if serializer.is_valid():
            serializer.save()
            return CustomeResponse(serializer.data, status=status.HTTP_201_CREATED)
        else:
