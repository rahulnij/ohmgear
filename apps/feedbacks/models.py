...
class Feedbacks(models.Model):
    ...
    def __unicode__(self):
        return '{"id":"%s","user_id":"%s","comment":"%s",bug_type":"%s"}' % \
            (self.id, self.user_id, self.comment, self.bug_type)
...
class ContactUs(models.Model):
    ...
    file_name = models.FileField(
        _("File Name"), upload_to='uploads/contact_file/', max_length=254, null=True, blank=True)
    created_date = models.DateTimeField(
        _("Created Date"), auto_now_add=True, auto_now=False)

    def __unicode__(self):
        return '{"id":"%s","feedback_category_id":"%s","feedback_category_subject_id":"%s","description":"%s"}' % \
            (self.id, self.feedback_category_id, self.feedback_category_subject_id, self.description)
