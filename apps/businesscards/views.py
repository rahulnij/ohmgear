            ...
            if bcard_id:
                ...
                if self.queryset: 
                    ...
                    for items in self.queryset:
                        if items.status == 1:
                           data['top'].append({"id":items.id,"front_back":items.front_back,"img_url":str(settings.DOMAIN_NAME)+str(settings.MEDIA_URL)+str(items.img_url)})
                        data['all'].append({"id":items.id,"front_back":items.front_back,"img_url":str(settings.DOMAIN_NAME)+str(settings.MEDIA_URL)+str(items.img_url)})
