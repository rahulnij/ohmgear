                        ...
                        for items in raw_data["json_data"]:
                            ...
                            if business_card_response["status"]:
                                ...
                                business_data.append(
                                    {"local_business_id": local_business_id, "bcard_id": bcard_id})
                            else:
                                business_data.append(
                    ...
                    if raw_data["json_data"]:
                        ...
                        for items in raw_data["json_data"]:
                            ...
                            if data['bcard_id']:
                                ...
                                business_data.append(
                                    {"local_business_id": local_business_id, "bcard_id": 'provide business card id to update'})
                        businesscard_copy['businesscard'][
                            position]['json_data'] = business_data
        ...
        for key, value in getkey.items():
            try:
                UserSetting.objects.filter(
                    user_id=request.user.id, setting_id__key=key).update(value=value)
